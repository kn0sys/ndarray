use kn0sys_ndarray::{arr2, arr3, aview1, aview2, concatenate, stack, Array2, Axis, ErrorKind, Ix1};

#[test]
fn concatenating()
{
    let a = arr2(&[[2., 2.], [3., 3.]]);
    let b = concatenate(Axis(0), &[a.view(), a.view()]).unwrap();
    ...
    let res = concatenate(Axis(1), &[a.view(), c.view()]);
    assert_eq!(res.unwrap_err().kind(), ErrorKind::IncompatibleShape);

    let res = concatenate(Axis(2), &[a.view(), c.view()]);
    assert_eq!(res.unwrap_err().kind(), ErrorKind::OutOfBounds);

    let res: Result<Array2<f64>, _> = concatenate(Axis(0), &[]);
...
{
    let a = arr2(&[[2., 2.], [3., 3.]]);
    let b = stack(Axis(0), &[a.view(), a.view()]).unwrap();
    ...
    let res = stack(Axis(1), &[a.view(), c.view()]);
    assert_eq!(res.unwrap_err().kind(), ErrorKind::IncompatibleShape);

    let res = stack(Axis(3), &[a.view(), a.view()]);
    assert_eq!(res.unwrap_err().kind(), ErrorKind::OutOfBounds);

    let res: Result<Array2<f64>, _> = stack::<_, Ix1>(Axis(0), &[]);
    assert_eq!(res.unwrap_err().kind(), ErrorKind::Unsupported);
}
